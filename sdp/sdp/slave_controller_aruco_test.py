...
class SlaveController(WebotsNode):
    ...
    def publish_god_odometry(self):
        ...
        if self.current_position[2] > 4:
            self.get_logger().info('Saving results')
            np.savetxt("/home/kopi22/aruco_test.csv", np.asarray(self.errors), delimiter=',')
            sys.exit()
    ...
    def merge_aruco_odometry(self, msg):
        ...
        dist_error = math.sqrt((msg.x-self.current_position[2])**2 + (msg.y-self.current_position[0])**2)
        self.errors.append([self.current_position[2], self.current_position[0], dist_error])

        self.get_logger().info(f'{len(self.errors)}')
